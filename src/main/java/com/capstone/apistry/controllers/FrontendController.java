...
public class FrontendController {
    ...
    public String request(@PathVariable Long id, Model model) {
        ...
        if (request == null) {
            // If the requested ID does not exist (for example, when clicking
            // a history item that points to a record which is not in this
            // environment's database), avoid a 500 error by redirecting
            // the user back to a fresh request page instead of rendering
            // the template with a null "request".
